    ...
    CircleMenu.prototype.init = function(){
        ...
        if(typeof self.options.direction === 'string'){
            ...
            if(dir){
				...
				{
					if ( self.options.item_primero == null )
					{
						self.menu_items = jQuery('.'+self.options.item_class+':not(:first-child)', self.element );
					}
					else
					{
						self.menu_items = jQuery('.'+self.options.item_class, self.element );
					}
				}
				else
				{
					...
					{
						self.menu_items = self.element.children('li');
					}
				}
        ...
        if(self.options.trigger === 'hover'){
            self.element.on('mouseenter',function(evt){
        		...
        		if ( self.options.div == true ) {
        			...
        			{
	        			item_primero = jQuery('.'+self.options.item_primero, self.element );
        			}
        		} else {
        			if ( self.options.item_primero == null )
        			{
            ...
            if ( self.options.div == true ) 
					  {
					  	if ( self.options.item_primero == null )
					  	{
			          set_other = jQuery('.'+self.options.item_class+':not(:nth-child('+index+'),:first-child)', self.element );
			          selected = jQuery('.'+self.options.item_class+':nth-child('+index+')');
	            }
	            else
	            {
	            	set_other = jQuery('.'+self.options.item_class+':not(:nth-child('+index+'), .'+self.options.item_primero+')', self.element );
			          selected = jQuery('.'+self.options.item_class+':nth-child('+index+')');
	            }
					  }
					  else 
	            ...
	            {
	            	set_other = self.element.children('li:not(:nth-child('+index+'), .'+self.options.item_primero+')');
			          selected = self.element.children('li:nth-child('+index+')');
	            }
					  }
    ...
    CircleMenu.prototype.initCss = function(){
        var self = this, 
            $items;
				...
				if ( self.element.hasClass('circle_menu') == false ) return;
        ...
        if (self.options.div == true )
        {
    			...
    			{
      			jQuery('.'+self.options.item_primero, self.element ).css({'z-index': 1000-self.options.depth});
    			}
        }
        else
        {
